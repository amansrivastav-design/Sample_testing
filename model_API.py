...
@app.get('/predict')
def predict(current_conditions=get_current_conditions(), model=loaded_model, X_scaler=loaded_scaler):
    ...
    return {"Optimal setpoints to minimize Fan Power " : {"RA Temp Setpoint" : optimal_setpoints['RA temperature setpoint'], "RA CO2 Setpoint" : optimal_setpoints['RA CO2 setpoint'], "SA Pressure Setpoint" : optimal_setpoints['SA Pressure setpoint'], "Predicted minimal Fan Power" : minimal_power},  "Corresponding optimal control commands " : {"RA Damper Control" : optimal_commands[0][0], "RA Temp control( Valve Feedback)" : optimal_commands[0][1], "SA Fan Speed Control" : optimal_commands[0][2]}}
